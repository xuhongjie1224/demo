###ssGSEA analysis of AF dataset
rm(list = ls()) 
load(file = "GSE50772step2output.Rdata")
library(dplyr)

exp <- exp[!duplicated(exp$symbol),]
rownames(exp)=exp$symbol
exp<- select(exp,-c(symbol,probe_id))
...
library(stringr)
group_list=ifelse(str_detect(colnames(res),"Control"),"Normal","SLE")
...
mydata$group <- ifelse(str_detect(rownames(mydata),"Control"),"Normal","SLE")
table(mydata$group)
